...
const SimpleFeatureCard: React.FC<SimpleFeatureCardProps> = ({ icon, title, description, caseStudy, animationDelay }) => {
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent side="top" className="max-w-xs z-[60]">
  ...
  return (
    ...
    <div className="flex flex-col min-h-screen bg-background text-foreground selection:bg-accent selection:text-accent-foreground">
      ...
      <main className="flex-1">
          ...
          <div className="container px-4 md:px-6 relative z-10">
            <div className="mx-auto max-w-3xl space-y-5 md:space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center items-center pt-4">
                <Button size="lg" asChild className="shadow-lg bg-gradient-to-r from-electric-teal to-neon-lime text-background font-bold text-base md:text-lg px-8 py-5 hover:opacity-90 transition-all duration-300 hover:scale-105 group pulse-cta-btn btn-interactive">
                  <Link href="/dashboard" className="flex items-center gap-2">
                    Launch Your First Agent Free
                    <Rocket className="h-5 w-5 group-hover:animate-bounce" />
                  </Link>
                </Button>
                <Button size="lg" variant="outline" asChild className="transition-all duration-300 hover:scale-105 hover:border-accent hover:text-accent px-8 py-5 text-base md:text-lg border-muted-foreground/50 text-foreground bg-background/30 backdrop-blur-sm btn-interactive">
                  <Link href="#video-demo-placeholder" className="flex items-center gap-2">
                    Watch 60s Demo <PlayCircle className="h-5 w-5"/>
                  </Link>
                </Button>
              </div>
            </div>
        ...
        <section className="section-light-accent w-full py-8 md:py-10 border-b border-border">
            <div className="container px-4 md:px-6 text-center">
                <h2 className="text-xs font-semibold uppercase tracking-wider text-primary mb-4">Powering AI Breakthroughs for Visionary Teams</h2>
                <div className="marquee w-full max-w-5xl mx-auto">
                  <div className="marquee-content flex items-center gap-10 md:gap-14">
                    {[...Array(2)].flatMap((_, repeatIndex) => [
                      ...
                      <div key={`${repeatIndex}-${index}`} className="flex-shrink-0 h-7 md:h-9 grayscale opacity-60 hover:grayscale-0 hover:opacity-100 transition-all duration-300 transform hover:scale-110">
                         <Image loading="lazy" src={`https://placehold.co/150x40/transparent/888888.png?text=${logo.name.replace(/\s/g,'+')}&font=nunito`} alt={`${logo.name} Logo`} width={150} height={40} className="object-contain h-full" data-ai-hint={logo.hint} />
                      </div>
                    )))}
                  </div>
                </div>
        ...
        <section ref={superpowersRef} id="superpowers" className={cn("scroll-reveal section-dark w-full py-12 md:py-20 lg:py-24", superpowersVisible && "visible")}>
          <div className="container px-4 md:px-6 text-center">
            <h2 className="font-headline mb-3 text-foreground">
              This Isn't Just Automation. This is <span className="gradient-text-on-dark">Your AI Workforce.</span>
            </h2>
            <p className="text-muted-foreground md:text-lg max-w-2xl mx-auto mb-10 md:mb-12">
              AutoBoss agents are more than tools—they're teammates. Here's how they deliver game-changing results:
            </p>
            <div className="grid md:grid-cols-1 lg:grid-cols-2 gap-6 max-w-5xl mx-auto text-left">
              <SimpleFeatureCard
        ...
        <section ref={howItWorksRef} id="how-it-works" className={cn("scroll-reveal section-light-accent w-full py-12 md:py-20 lg:py-24", howItWorksVisible && "visible")}>
          <div className="container px-4 md:px-6 text-center">
            <h2 className="font-headline mb-3 text-card-foreground">
            ...
            <div className="relative mx-auto max-w-5xl grid md:grid-cols-3 gap-6 md:gap-8 items-start">
                ...
                {[
                  ...
                  return (
                    <article key={step.title} ref={stepRef} className={cn("scroll-reveal relative flex flex-col items-center gap-2 p-6 rounded-lg bg-background shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 text-foreground z-10", step.animationDelay, stepIsVisible && "visible")}>
                        ...
                        <div className="mt-8 mb-1">{step.icon}</div>
            ...
            <div ref={useIntersectionObserver(sectionObserverOptions)[0]} className={cn("scroll-reveal mt-12 md:mt-16 max-w-3xl mx-auto", useIntersectionObserver(sectionObserverOptions)[1] && "visible", "delay-450")}>
        ...
        <section ref={videoDemoRef} id="video-demo-placeholder" className={cn("scroll-reveal section-dark w-full py-12 md:py-20 text-center", videoDemoVisible && "visible")}>
            <div className="container px-4 md:px-6">
                ...
                <p className="text-muted-foreground md:text-lg max-w-xl mx-auto mt-3 mb-6">Watch how easy it is to build and deploy an intelligent AI teammate.</p>
        ...
        <section ref={socialProofRef} id="social-proof" className={cn("scroll-reveal section-light-accent w-full py-12 md:py-20 lg:py-24", socialProofVisible && "visible")}>
            <div className="container px-4 md:px-6 text-center">
        ...
        <section ref={humanizeRef} className={cn("scroll-reveal section-dark w-full py-12 md:py-20 lg:py-24", humanizeVisible && "visible")}>
          <div className="container px-4 md:px-6 text-center">
            <h2 className="font-headline text-foreground">
              Built by Humans, for <span className="gradient-text-on-dark">Your Breakthroughs</span>.
            </h2>
            <p className="text-muted-foreground md:text-lg max-w-2xl mx-auto mt-3 mb-10">
            ...
            <div className="max-w-xl mx-auto bg-card p-6 rounded-lg shadow-xl transform hover:scale-103 transition-transform text-card-foreground">
              <p className="italic text-sm text-muted-foreground">"I started AutoBoss from my dorm room, frustrated by clunky automation tools. My vision: a platform so intuitive, anyone could build truly intelligent AI agents that *actually work*. We're just getting started."</p>
            ...
            <aside ref={useIntersectionObserver(sectionObserverOptions)[0]} className={cn("scroll-reveal mt-10", useIntersectionObserver(sectionObserverOptions)[1] && "visible", "delay-250")}>
        ...
        <section ref={finalCtaRef} className={cn("scroll-reveal section-cta-final w-full py-16 md:py-24 lg:py-28", finalCtaVisible && "visible")}>
          <div className="container px-4 md:px-6 text-center">
            <div className="mx-auto max-w-xl space-y-5 bg-card/80 dark:bg-background/60 backdrop-blur-md p-8 md:p-10 rounded-xl shadow-2xl">
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center items-center pt-4">
                <Button size="lg" asChild className="shadow-xl bg-gradient-to-r from-electric-teal to-neon-lime text-background font-bold text-md px-8 py-5 hover:opacity-90 transition-all duration-300 hover:scale-105 group w-full sm:w-auto pulse-cta-btn btn-interactive">
                  <Link href="/dashboard" className="flex items-center gap-2">
                    Start Building Free
                    <Rocket className="h-4 w-4 group-hover:animate-bounce" />
                  </Link>
                </Button>
                <Button size="lg" variant="outline" asChild className="transition-all duration-300 hover:scale-105 hover:border-accent hover:text-accent px-8 py-5 text-md text-foreground border-muted-foreground/50 bg-background/30 dark:bg-card/30 backdrop-blur-sm w-full sm:w-auto btn-interactive">
                  <Link href="mailto:demo@autoboss.dev?subject=AutoBoss%20Demo%20Request" className="flex items-center gap-2">
      ...
      <footer className="w-full py-6 border-t border-border/30 bg-background text-center">
        <div className="container px-4 md:px-6 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <nav className="flex flex-wrap justify-center gap-3 sm:gap-4 mt-3 sm:mt-0">
